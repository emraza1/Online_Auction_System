...
<div class="img-big-wrap">
    <div> <a href="#"><img class="single_img" style="width:430px; height:480px;" src="images/elec_img/<?=$result['id']?>.jpg"></a></div>
</div> <!-- slider-product.// -->
...
<article class="card-body p-5">
	<h3 class="title mb-3"><?= $result['elec_title']; ?></h3>

<p class="price-detail-wrap"> 
	<span class="price h3 text-warning"> 
		<span class="currency">Starting Price US $</span><span class="num"><?= $result['elec_price']; ?></span>
	</span> 
	<span></span> 
</p> <!-- price-detail-wrap .// -->
...
<?php    include '_inc/timer_js.php'; ?>
        ...
        if (($days_remaining <= 0) && ($hours_remaining <= 0) && ($minutes_remaining <= 0) && ($seconds_remaining <= 0)){
								...
								$id = $_GET['id'];
								$sql = "SELECT username,price,u_id FROM bid_elec WHERE id= $id ORDER BY Price DESC LIMIT 1";
								$rest = $conn -> query($sql);
								$count = 1;
   ...
   $itm = $_GET['id']; 
     $sql2="SELECT * FROM users INNER JOIN electronics ON users.u_id=electronics.u_id WHERE id='$itm'";
$res1 = $conn -> query($sql2);
                if ($res1 -> num_rows > 0) {
                      while ($row = $res1 -> fetch_assoc()) {        
                       ...
                       $e_mail = $row["email"];
                       $phone = $row["phone"];
                        $title = $row["elec_title"];
                       $elec_com = $row["elec_company"];
                       $elec_mod = $row["elec_model"];
                       $cur = $row["cur_price"];
                       $elec = $row["elec_price"];
                       }
                    }
   ...
   $result1 = $conn -> query($sql1);
    ...
    if ($result1 -> num_rows > 0) {
       ...
       sendEmail($row["username"],$row["email"],  $title . ' ' . $elec_com . ' ' .  $elec_mod . ' '. $user . ' ' . $phone . ' ' . $e_mail . ' ' . $cur . ' ');
                    }
                  }
...
<dl class="item-property">
  <dt>Type</dt>
  <dd><p><?= $result['elec_type']; ?></p></dd>
</dl>
<dl class="param param-feature">
  <dt>Company</dt>
  <dd><?= $result['elec_company']; ?></dd>
</dl>  <!-- item-property-hor .// -->

<dl class="param param-feature">
  <dt>Model</dt>
  <dd><?= $result['elec_model']; ?></dd>
</dl>  <!-- item-property-hor .// -->
 ...
 if (isset($_POST['submit']))
  {
     ...
     $name = $_SESSION['login_user'];
     $id = $_GET['id'];
         $sql="INSERT INTO bid_elec (username, u_id, price, id) VALUES ('$name', '$u_id', '$amount', '$id')";
         mysqli_query($conn,$sql);
          $amount = $_REQUEST['bAmount'];
         $query = "UPDATE `electronics` SET `cur_price`='$amount'
         WHERE id='$id'";
...
?>
        ...
        <dt>
						...
						<dd>
							...
							<?php
								$id = $_GET['id'];
								$sql = "SELECT username,price FROM bid_elec WHERE id= $id ORDER BY Price DESC";
