...
<div class="img-big-wrap">
    <div> <a href="#"><img class="single_img" style="width:430px; height:480px;" src="images/jew_img/<?=$result['id']?>.jpg"></a></div>
</div> <!-- slider-product.// -->
...
<article class="card-body p-5">
	<h3 class="title mb-3"><?= $result['jew_type']; ?></h3>

<p class="price-detail-wrap"> 
	<span class="price h3 text-warning"> 
		<span class="currency">Starting Price US $</span><span class="num"><?= $result['jew_price']; ?></span>
	</span> 
	<span></span> 
</p> <!-- price-detail-wrap .// -->
...
<?php    include '_inc/timer_js.php'; ?>
        ...
        if (($days_remaining <= 0) && ($hours_remaining <= 0) && ($minutes_remaining <= 0) && ($seconds_remaining <= 0)){
								...
								$id = $_GET['id'];
								$sql = "SELECT username,price,u_id FROM bid_jew WHERE id= $id ORDER BY Price DESC LIMIT 1";
								$rest = $conn -> query($sql);
								$count = 1;
   ...
   $itm = $_GET['id']; 
     $sql2="SELECT * FROM users INNER JOIN jewellery ON users.u_id=jewellery.u_id WHERE id='$itm'";
$res1 = $conn -> query($sql2);
   ...
   $result1 = $conn -> query($sql1);
    ...
    if ($result1 -> num_rows > 0) {
       ...
       sendEmail($row["username"],$row["email"],  $type . ' ' . $jew_mat . ' ' .  $brand . ' '. $user . ' ' . $phone . ' ' . $e_mail . ' ' . $cur . ' ');
                    }
                  }
...
<dl class="item-property">
  <dt>Material</dt>
  <dd><p><?= $result['jew_material']; ?></p></dd>
</dl>

<dl class="item-property">
  <dt>Brand</dt>
  <dd><p><?= $result['jew_brand']; ?></p></dd>
</dl>
 ...
 if (isset($_POST['submit']))
  {
     ...
     $name = $_SESSION['login_user'];
     $id = $_GET['id'];
         $sql="INSERT INTO bid_jew (username, u_id, price, id) VALUES ('$name', '$u_id', '$amount', '$id')";
         mysqli_query($conn,$sql);
          $amount = $_REQUEST['bAmount'];
         $query = "UPDATE `jewellery` SET `cur_price`='$amount'
         WHERE id='$id'";
...
?>
        ...
        <dt>
						...
						<dd>
							...
							<?php
								$id = $_GET['id'];
								$sql = "SELECT username,price FROM bid_jew WHERE id= $id ORDER BY Price DESC";
